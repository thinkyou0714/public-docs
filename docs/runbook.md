...
13. `fail_on_dod_drop=true` で fail した場合、`sync-dod-drop` ラベル付き Issue が tenant 単位で自動起票される（既存 open があれば新規作成せず同一Issueにコメント追記、DoD低スコア上位3件 + 失敗チェックID（日本語ラベル付き）+ 失敗頻度ベースの推奨修正順 + 変更ファイル最大20件を記録）
